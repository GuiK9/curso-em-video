...
function finalizar() {
    ...
    if (nl == 0){
        ...
        ret.innerHTML += `O maior valor informado é ${trat[0]} <br>` 
        ret.innerHTML += `O menor valor informado é ${trat[1]} <br>`
        ret.innerHTML += `A soma dos valores é ${soma} <br>`
        ret.innerHTML += `a média dos valores informados é ${media} <br>` 
    }
}